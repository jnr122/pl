...
will be treated accordincg to UVM's Code of Academic Integrity.
...
substValue x v e = case e of
  ...
  L5.LetE y e1 e2 ->
    if x == y
      then L5.LetE y (substValue x v e1) e2
      else L5.LetE y (substValue x v e1) (substValue x v e2)
  L5.FunE y e' ->
    if x == y
       then L5.FunE y e'
       else L5.FunE y (substValue x v e')
  L5.AppE e1 e2 -> L5.AppE (substValue x v e1) (substValue x v e2)
...
interpWithSubst e = case e of
  ...
  L5.IfE e1 e2 e3 -> case interpWithSubst e1 of
    ...
    _ -> L5.BadSA
  L5.VarE _x -> L5.BadSA
  L5.LetE x e1 e2 -> case interpWithSubst e1 of
    L5.ValueSA v -> interpWithSubst (substValue x v e2)
    _ -> L5.BadSA
  L5.FunE x e ->
    if Set.isSubsetOf (freeVarsExpr e) (Set.singleton x)
       then L5.ValueSA (L5.FunSV x e)
       else L5.BadSA
  L5.AppE e1 e2 -> case (interpWithSubst e1, interpWithSubst e2) of
    (L5.ValueSA (L5.FunSV x e'), L5.ValueSA v) -> interpWithSubst (substValue x v e')
    _ -> L5.BadSA
